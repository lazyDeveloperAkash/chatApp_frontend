...
import { asyncAvatar, asyncUpdateUser } from '@/store/Actions/userActions';
            ...
            <div className='w-full h-full md:w-[30vmax] md:h-[80vh] rounded-2xl px-4 bg-black'>
                ...
                <form onSubmit={submitHandler}>
                    <div className="flex flex-col gap-3 w-full text-white">
                        ...
                        </div>) : (<div className='flex items-center justify-between text-2xl'><h1>{name}</h1> <MdEdit onClick={() => { setWriteName(true); setIsDisabled(false) }} /> </div>)}
                        {writeContact ? (<div>
                        ...
                        </div>) : (<div className='flex items-center justify-between text-xl'><h1>{contact}</h1> <MdEdit onClick={() => { setWriteContact(true); setIsDisabled(false) }} /> </div>)}
                        {writeEmail ? (<div>
                        ...
                        </div>) : (<div className='flex items-center justify-between text-lg'><h1>{email}</h1> <MdEdit onClick={() => { setWriteEmail(true); setIsDisabled(false) }} /> </div>)}
                        <div className="flex items-center flex-col gap-2 my-3">
                            <button type='submit' disabled={isDisabled} className={`px-20 py-2 rounded-3xl ${isDisabled ? "bg-yellow-200" : "bg-yellow-300"}`}>Save Edit</button>
